...
struct ContentView : View {
    ...
    var body: some View {
        VStack {
            Text("Local solar time: ").bold()
            Text("\(localSolarTime.localSolarTime)").fontWeight(.light)
            Spacer()
            ...
            Spacer()
            Text("Distance from the North Pole: ").bold()
            Text("\(self.distanceNorth.meterDistance) km").fontWeight(.light)
        }
    }
}
